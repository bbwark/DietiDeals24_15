...
fun FavouritesView(user: User) {
    ...
    Column(Modifier.fillMaxSize()){
        ...
        when (tabIndex) {
            0 -> FavouriteAuctionsList(auctions = user.favouriteAuctions.filter { it.endingDate.isAfter(LocalDate.now()) }.toTypedArray()) //ActiveAuctions
            1 -> FavouriteAuctionsList(auctions = user.favouriteAuctions.filter { it.endingDate.isBefore(LocalDate.now().plusDays(1)) }.toTypedArray()) //FinishedAuctions
        }
    }
}
...
@Composable
fun FavouriteAuctionsList(auctions: Array<Auction>) {
    auctions.forEach { auct ->
        Text(text = auct.endingDate.toString())
    }
}
